"""Live open/active app awareness via process names only (no window titles)."""
...
# Shell / system hosts that are almost always present and not useful to mention.
...
def friendly_app_name(exe_path_or_name: str) -> str:
    """Map an exe path or basename to a short display name."""
...
def is_noise_process(exe_path_or_name: str) -> bool:
    """Return True for shell/system hosts we should not surface to dialogue."""
    return Path(exe_path_or_name).name.lower() in IGNORED_EXE_NAMES
...
def capture_open_apps() -> AppSnapshot:
    """Enumerate visible top-level window apps on Windows; empty elsewhere."""
